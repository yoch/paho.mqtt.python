...
path = Path(__file__).with_name("apply_hardening_tests.py")
text = path.read_text()
marker = '    tls = ROOT / "tests/unit/test_tls.py"\n'
if text.count(marker) != 1:
    raise RuntimeError("TLS insertion marker changed")
contract = r'''
    write(
        "tests/unit/test_performance_contracts.py",
        r''' + "'''" + r'''
        from pathlib import Path
...
'''
path.write_text(text.replace(marker, contract + marker, 1))
